...
% Average reference after adding 0-filled channel to preserve data
% rank
...
eegplot(EEG.data,'winlength',15,'srate',EEG.srate,'spacing',150, 'events',EEG.event,...
...
if strcmpi(reply,'y')
    ...
    for iSegment = 1:size(badData,1)
        EEG.data(:,badData(iSegment,1):badData(iSegment,2)) = 0;
    end
