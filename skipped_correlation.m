...
if fig_flag ~= 0
    ...
    if p > 1
        ...
        scatter(a{1},b{1},20,[0 0.4470 0.7410],'fill');                     %COLOR SCATTER
        grid on; hold on;
        hh = lsline; set(hh,'Color',[0.6350 0.0780 0.1840],'LineWidth',2);     %COLOR SLOPE
        try
            [XEmin, YEmin] = ellipse(a{column},b{column});
            plot(real(XEmin), real(YEmin),'LineWidth',1,'Color',[0.6350 0.0780 0.1840]); %COLOR ELLIPSE
...
%         title(M,'Fontsize',16);
        ...
        scatter(x(outid{1}),y(outid{1}),20,[0.6350 0.0780 0.1840],'filled');       %COLOR OUTLIERS
        ...
        if nargout>4 && sum(~isnan(CIpslope))==2
            % add CI
            y1 = refline(CIpslope(1),CIpintercept(1)); set(y1,'Color',[0.6350 0.0780 0.1840]);     %COLOR CI
            y2 = refline(CIpslope(2),CIpintercept(2)); set(y2,'Color',[0.6350 0.0780 0.1840]);     %COLOR CI
            y1 = get(y1); y2 = get(y2);
            ...
            hold on; fillhandle=fill(xpoints,filled,[0.6350 0.0780 0.1840]);
            set(fillhandle,'EdgeColor',[0.6350 0.0780 0.1840],'FaceAlpha',0.2,'EdgeAlpha',0.8);%set edge color
    ...
    if strcmp(answer,'yes')
        for f = 1:p
            ...
            if nargout >4
                if ~isnan(r(f))
                    index = 3; %subplot(1,3,1);
                else
                    index = 2; %subplot(1,2,1);
                end
            ...
            scatter(a{f},b{f},20,[0 0.4470 0.7410],'fill');     %COLOR SCATTER
            grid on; hold on;
            hh = lsline; set(hh,'Color',[0.6350 0.0780 0.1840],'LineWidth',1);     %COLOR SLOPE
            try
                [XEmin, YEmin] = ellipse(a{f},b{f});
                plot(XEmin, YEmin,'LineWidth',1,'Color',[0.6350 0.0780 0.1840]);    %COLOR ELLIPSE
                MM = [min(XEmin) max(XEmin) min(YEmin) max(YEmin)];
            catch ME
...
%             title('Outlier detection','Fontsize',16);
            ...
            % add outliers and scale axis
            scatter(x(outid{f},f),y(outid{f},f),20,[0.6350 0.0780 0.1840],'filled');       %COLOR OUTLIER
            ...
            scatter(xrank,yrank,20,[0 0.4470 0.7410],'fill'); grid on; hold on                  %COLOR SCATTER
            hh = lsline; set(hh,'Color',[0.6350 0.0780 0.1840],'LineWidth',1); axis tight          %COLOR SLOPE
...
%             title(M,'Fontsize',16);
            ...
            if nargout>4 && sum(isnan(CIpslope(:,f))) == 0
                % add CI
                y1 = refline(CIsslope(1,f),CIsintercept(1,f)); set(y1,'Color',[0.6350 0.0780 0.1840]);     %COLOR CI
                y2 = refline(CIsslope(2,f),CIsintercept(2,f)); set(y2,'Color',[0.6350 0.0780 0.1840]);     %COLOR CI
                y1 = get(y1); y2 = get(y2);
                xpoints=[[y1.XData(1):y1.XData(2)],[y2.XData(2):-1:y2.XData(1)]];
                step1 = y1.YData(2)-y1.YData(1); step1 = step1 / (y1.XData(2)-y1.XData(1));
                step2 = y2.YData(2)-y2.YData(1); step2 = step2 / (y2.XData(2)-y2.XData(1));
