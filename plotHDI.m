...
set(fillhandle,'EdgeColor', color1,'FaceAlpha',0.3,'EdgeAlpha',0.5);
p2 = plot(xAxis, est2,'LineWidth',2,'Color', color2);
fillhandle = fill([xAxis fliplr(xAxis)],[HDI2(1,:) fliplr(HDI2(2,:))], color2);
set(fillhandle,'EdgeColor', color2,'FaceAlpha',0.3,'EdgeAlpha',0.5);
...
%PLOT MEAN AND 95% HDI OF THE DIFFERENCE
subplot(2,1,2)
plot(xAxis, est3,'LineWidth',2,'Color', color3); hold on;
fillhandle = patch([xAxis fliplr(xAxis)], [HDI3(1,:) fliplr(HDI3(2,:))], color3);
set(fillhandle,'EdgeColor', color3,'FaceAlpha',0.3,'EdgeAlpha',0.5);
set(gca,'FontSize',12,'layer','top');
grid on; axis tight;  %box on
xlabel('Frequency (Hz)','FontSize',12)
% xlabel('Time (2-s sliding windows)','FontSize',12)
ylabel('MSE Difference','FontSize',12); 
xlabel('Scales');
% ylabel('Amplitude difference','FontSize',12);
% title([plot1Title ' (' type ' and 95% HDI']); 
title('Difference (Trimmed mean and 95% HDI)');
